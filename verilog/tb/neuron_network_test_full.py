        ...
        for row_i in range(32):
            for col_i in range(32):
                ...
                if layer == 0:
                    ...
                    # Each gesture frame is NUM_STIMULI_WORD (119) 32-bit words; offset by pic to get the right frame
                    stimuli_val = stimuli[(pic * NUM_STIMULI_WORD) + (axon // 2)]
...
async def neuron_network_test(dut): 
    ...
    correct_labels_raw = read_matrix_from_file("./mem/testbench/tb_correct.txt")
    correct_labels = [list_to_binary(row) for row in correct_labels_raw]
    ...
    for core_layer_0 in range(NUM_CORES_LAYER_0):
        ...
        await program_layer_connections(dut, core_layer_0, connection_layer_0, NUM_NEURONS_LAYER_0)
    ...
    for core_layer_1 in range(NUM_CORES_LAYER_1):
        ...
        # 1. Configuration (ONCE)
        await program_layer_connections(dut, core_layer_1, connection_layer_1, NUM_NEURONS_LAYER_1)
    ...
    for core_layer_2 in range(NUM_CORES_LAYER_2):
        ...
        # 1. Configuration (ONCE)
        await program_layer_connections(dut, core_layer_2, connection_layer_2, NUM_NEURONS_LAYER_2)
    ...
    predict_classes = calculate_majority_class(spike_out_layer_2)

    correct_count = 0
    import csv
    with open("results.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["pic", "predicted_class", "true_class", "correct"])
        for pic in range(SUM_OF_PICS):
            true_class = correct_labels[pic]
            predicted = predict_classes[pic]
            is_correct = int(predicted == true_class)
            correct_count += is_correct
            writer.writerow([pic, predicted, true_class, is_correct])
            print(f"  Pic {pic:4d}: predicted={predicted:2d}  true={true_class:2d}  {'CORRECT' if is_correct else 'WRONG'}")

    accuracy = correct_count / SUM_OF_PICS * 100
    print(f"\nAccuracy: {correct_count}/{SUM_OF_PICS} = {accuracy:.2f}%")
